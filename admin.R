...
fit4=lm(formula= quantity~avgavailability+merchnumless950+if_june18+uv+pv,data=data)
summary(fit4)
anova(fit3,fit4)
...
fit2=lm(formula = quantity~cumlogisticscore+cumservicescore+if_Mon+if_Thu+if_Wed+view+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2) #fit2

fit3=lm(formula = quantity~cumlogisticscore+cumservicescore+if_Mon+if_Wed+view+uv+pv, data =data)
summary(fit3)
anova(fit2,fit3)
#fit3 wins
...
#require(HH)
#require(lmtest)
#p<-predict(fit3,h=31,data=train$quantity)
#accuracy(p)
#plot(p)
#fitted(p)
#lines(fitted(p),col = "blue")
#accuracy(p)
...
fit2=lm(formula = quantity~cumlogisticscore+cumorderscore+avgavailability+merchnumless950+if_Thu+noofsellersbr+if_june18+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2) #fail to reject stop with fit2
#fit2 wins
...
fit2=lm(formula = quantity~price+if_june18+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2) #fail to reject fit2 wins
...
fit2=lm(formula = quantity~maxprice+avgavailability+merchnumless950+if_Sun+if_Mon+noofsellerssc+if_june18+pv, data =data)
summary(fit2)
anova(fit1,fit2)
#fit2 wins
...
fit2=lm(formula = quantity~price+maxprice+minprice+cumlogisticscore+avgavailability+merchnumless950+if_june18+view+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2)
#fit2 wins
...
fit2=lm(formula = quantity~price+maxprice+cumlogisticscore+cumservicescore+merchnumless950+if_Sun+if_Mon+if_Tue+noofsellerssc+noofsellersbr+if_june18+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2)

fit3=lm(formula = quantity~price+maxprice+cumservicescore+merchnumless950+if_Sun+if_Mon+if_Tue+noofsellerssc+noofsellersbr+if_june18+uv+pv, data =data)
summary(fit3)
anova(fit2,fit3)
#fit3 wins
...
fit1=step(lm(formula = quantity~price+maxprice+minprice+avgprice+avgavailability+merchnumless950+if_Sun+if_Mon+if_Tue+if_Wed+if_Thu+if_Fri+if_Sat+noofsellerssc+noofsellersbr+if_june18+if_chinese+view+uv+pv, data =data))
summary(fit1) 
#bu sefer cum'ler NA

#!!!!!!!!!!!!!!!
fit2=lm(formula = quantity~merchnumless950+if_Wed+if_june18+view+pv, data =data)
summary(fit2)
anova(fit1,fit2) 
#NEGATIVE R^2 SOMETHING'S TOO WRONG!!!
...
fit1=step(lm(formula = quantity~price+avgavailability+merchnumless950+if_Sun+if_Mon+if_Tue+if_Wed+if_Thu+if_Fri+if_Sat+noofsellerssc+noofsellersbr+if_june18+if_chinese+view+uv+pv, data =data))
summary(fit1)#SIMILAR TO ABOVE CASE
...
fit2=lm(formula = quantity~cumlogisticscore+avgavailability+if_Tue+if_Fri+if_june18+view+uv+pv, data =data)
summary(fit2)
anova(fit1,fit2)
#fit2 wins :)
