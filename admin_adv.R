...
products[,pv:=app_pv+pc_pv]
products[,uv:=app_uv+pc_uv]
tr_sub_cat=merge(tr[,.(if_cainiao, merchant_id, date, item_id, quantity, amount, price),], 
                 products[, .(date, item_id, sub_category_id, pv, uv, brand_id)], by=c("date", "item_id"))

#inventory_v1=inventory[, .(beg_inv=sum(beg_inv)), by=c("item_id", "date")]
...
dummy<-dummy[availability<950,dummy:=1,by=c("date", "sub_category_id", "brand_id")]
dummy<-dummy[availability>=950,dummy:=0,by=c("date", "sub_category_id", "brand_id")]
unique(dummy[,.(merchantnumless950:=sum(dummy)),by=c("brand_id","date","sub_category_id")])

tr_sc_brand=tr_sc[,.(quantity=sum(quantity),price=mean(price),view=(sum(pv)/sum(uv)),logisticscore=mean(logisticscore),
                     orderscore=mean(orderscore),servicescore=mean(servicescore), availability=sum(availability)),by=.(date, brand_id, sub_category_id)]
#Simgenin kodu

tr_sc_brand[,day:=weekdays(tr_sc_brand$date, abbreviate = TRUE)]
dummy=tr_sc_brand[,.(quantity=sum(quantity)),by=c("date","day")]
ggplot(tr_sc_brand[,.(quantity=sum(quantity)),by=c("date","day")],aes(x=date,y=quantity, colour=day, group=day))+geom_line()+geom_point()
...
#Þurayý üþenmeden yazana alkýþ (alkýþ emojisi)

tr_sc_brand=tr_sc_brand[order(date)]
...
#Ayrýca Nesliciðe de alkýþ bence arada unutulanlar olmuþ.
...
sc_250_fit_=lm(formula = quantity~logisticscore+orderscore+servicescore+price+view+beg_inv, data = tr_sc_brand[sub_category_id==250 & brand_id==232,])
summary(sc_250_fit_)
# 
# #stepwise regression
# sc_250_fit_step=step(lm(formula = quantity~logisticscore+orderscore+servicescore+price+view+beg_inv, data = tr_sc_brand[sub_category_id==250 & brand_id==232,]))
...
tr_sc_brand=merge(tr_sc_brand,noofsellers_subcat,by=c("date", "sub_category_id"))
tr_sc_brand=merge(tr_sc_brand,noofsellers_brand,by=c("date", "brand_id"))
...
temp_lagged=temp[,view := shift(view, 1, type="lag")]
